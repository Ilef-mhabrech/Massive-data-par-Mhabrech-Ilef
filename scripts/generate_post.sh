...
FOLLOWERS=20           # nombre de followees par user (fixe)
...
# ============================================================
# 1) Calculer combien de posts on a déjà
# ============================================================
echo "📊 Comptage des posts existants…"
...
echo "   → Objectif : ${TOTAL_POSTS} posts (${POSTS_PER_USER} par user)"

if (( CURRENT_POSTS >= TOTAL_POSTS )); then
    echo "✅ Déjà au moins ${TOTAL_POSTS} posts. Pas besoin de reseed."
    MISSING=0
else
    MISSING=$(( TOTAL_POSTS - CURRENT_POSTS ))
    echo "🌱 Il manque ${MISSING} posts, on va les créer…"

    # On insère en batch pour éviter un énorme seed d'un coup
    BATCH_SIZE=50000
    REMAINING=$MISSING
...
# Petit récap après seed
echo "📊 Vérification du nombre final de posts…"
FINAL_POSTS=$(
python3 - << 'EOF'
from google.cloud import datastore
...
echo "   → Posts après seed : ${FINAL_POSTS}"

# ============================================================
# 2) BENCHMARK (50 users simultanés)
# ============================================================
echo "🚀 Benchmark…"
